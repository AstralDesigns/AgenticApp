...
export default function UserTerminal() {
  ...
  // Initialize terminal - Dependency on containerReady only, NOT isVisible
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (window.electronAPI?.pty) {
        ...
        window.electronAPI.pty.create({
          ...
          // Handle input
          term.onData((data) => {
            // Filter OSC 11 response (Background Color) generated by xterm-addon-image
            // This prevents the query response from being echoed to the shell prompt
            if (data.includes(']11;rgb:') || data.includes(']11;?') || data === '\x07') {
              return;
            }
